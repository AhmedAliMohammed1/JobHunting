...
export function SearchWorkspace() {
  ...
  return <div className="search-workspace">
    <form className="search-bar" onSubmit={(event) => { event.preventDefault(); search(); }}><Search size={20} /><input aria-label="Search roles" value={query} onChange={(event) => setQuery(event.target.value)} disabled={!hydrated} required /><button type="submit" disabled={!hydrated || loading}>{loading ? "Searching…" : "Search"}</button></form>
    <div className="search-toolbar"><button className="text-button" type="button" onClick={() => setFiltersOpen((open) => !open)} aria-expanded={filtersOpen} disabled={!hydrated}><SlidersHorizontal size={16} /> Filters</button><strong role="status">{notice}</strong><button className="secondary-button" type="button" onClick={saveSearch} disabled={!hydrated}>Save search</button></div>
